...
const PHONE_ACTIONS = new Set<PhoneAction>(['contact-resolve','sms','calendar-read','hold','reentry-ack'])

function phoneAction(pathname: string, prefix: string): PhoneAction | null {
  if (!pathname.startsWith(prefix)) return null
  const action = pathname.slice(prefix.length) as PhoneAction
  return PHONE_ACTIONS.has(action) ? action : null
}
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    try {
      if (url.pathname === '/health' && req.method === 'GET') return json({ ok: true, service: 'caroline_phone', release: '3.11.0' }, 200, requestId)
      ...
      const runtimeAction = phoneAction(url.pathname, '/runtime/phone/')
      if (runtimeAction) return await handleRuntimePhoneAction(req, env, requestId, runtimeAction)

      const elevenLabsToolAction = phoneAction(url.pathname, '/elevenlabs/tools/')
      if (elevenLabsToolAction) return await handleRuntimePhoneAction(req, env, requestId, elevenLabsToolAction)

      if (url.pathname === '/webhook/elevenlabs') return await handleElevenLabsWebhook(req, env, requestId)
      if (url.pathname.startsWith('/twilio/') || url.pathname.startsWith('/webhook/twilio')) {
        return await handleTwilioWebhook(req, env, requestId)
      }
