...
export async function verifyElevenLabsSignature(rawBody: string, header: string | null, secret: string | undefined): Promise<{ ok: boolean; reason?: string }> {
  ...
  if (Math.abs(Math.floor(Date.now() / 1000) - timestamp) > MAX_SIGNATURE_AGE_SECONDS) return { ok: false, reason: 'stale_signature' }
  const expected = await hmacSha256Hex(secret, `${parsed.timestamp}.${rawBody}`)
  return parsed.signatures.some((sig) => timingSafeEqual(expected, sig)) ? { ok: true } : { ok: false, reason: 'invalid_signature' }
}
