...
export function handleStatus(env: Env, requestId: string): Response {
  ...
  return json({
    ok: true,
    service: 'caroline_event_worker',
    release: '3.11.0',
    role: 'cloudflare_caroline_runtime_boundary',
    ...
    configured: {
      ...
      core_runtime_key: Boolean(env.CORE_RUNTIME_KEY),
      ...
      events_raw_r2: Boolean(env.CAROLINE_EVENTS_RAW),
      transcripts_r2: Boolean(env.CAROLINE_TRANSCRIPTS),
      artifacts_r2: Boolean(env.CAROLINE_ARTIFACTS),
      media_r2: Boolean(env.CAROLINE_MEDIA),
      caroline_db: Boolean(env.CAROLINE_DB),
      event_delivery_queue: Boolean(env.EVENT_DELIVERY),
      twilio_auth_token: Boolean(env.TWILIO_AUTH_TOKEN),
      open_router_key: Boolean(env.OPEN_ROUTER_KEY),
    },
    ...
    storage_roles: {
      ...
      r2_events_raw: 'verified_event_payload_staging',
      r2_transcripts: 'transcript_object_storage',
      r2_artifacts: 'generated_artifact_storage',
      r2_media: 'media_object_storage',
      d1: 'cloudflare_relational_runtime_store',
