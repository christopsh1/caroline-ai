...
test('runtime init rejects requests without the edge runtime key', async () => {
  ...
  const response = await handleRuntimeInit(req, { CAROLINE_KEY: runtimeKey }, 'req-1')
...
test('runtime retrieval returns a sanitized authorized envelope', async () => {
  const originalFetch = globalThis.fetch
  ;(globalThis as any).fetch = async () => coreResponse({
    schema_version: '1', authorized: true, context: 'Relevant context', secret_debug: 'must-not-pass',
  ...
  try {
    ...
    const response = await handleRuntimeRetrieve(req, { CAROLINE_KEY: runtimeKey, CORE_RUNTIME_URL: 'https://core.example.test', CORE_RUNTIME_KEY: coreKey }, 'req-3')
    assert.equal(response.status, 200)
    const body = await response.json() as any
...
test('runtime retrieval requires conversation_id for conversation-bound authorization', async () => {
  const req = new Request('https://edge.test/runtime/retrieve', {
    method: 'POST', headers: { 'Content-Type': 'application/json', 'x-caroline-key': runtimeKey },
    body: JSON.stringify({ caller_phone: 'caller-fixture', interaction_mode: 'inbound_external', current_query: 'What did we discuss?' }),
  })
  const response = await handleRuntimeRetrieve(req, { CAROLINE_KEY: runtimeKey, CORE_RUNTIME_URL: 'https://core.example.test', CORE_RUNTIME_KEY: coreKey }, 'req-4')
