...
The live Worker itself remains in Cloudflare. Removing automation does not delete the deployed Worker, its routes, secrets, Durable Objects, or other production resources.
...
| `cloudflare/.env.example` | GitHub Actions deployment wording; unused OpenRouter live-call key inventory | Documentation / environment template | Yes | Updated for Dashboard/local Wrangler. OpenRouter removed from live phone secret inventory. |
...
| `cloudflare/caroline_phone/README.md` | Previous deployment assumptions and obsolete architecture | Documentation | Yes | Rewritten for ElevenLabs-native live runtime and direct deployment only. |
...
Use the existing `caroline-phone` Worker directly. Edit code/bindings/secrets through the Worker dashboard. Do not import or connect a repository.
...
Runtime secrets remain Cloudflare Worker Secrets. Local Cloudflare authentication must not be stored in project files.
...
- existing Worker secrets, custom domains/routes, Durable Object data, KV, R2, D1 or queues;
