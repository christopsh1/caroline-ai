...
8. Run `npm run check`; current v3.8 local baseline must pass the complete local contract suite before deployment.
...
12. Verify the implemented Twilio request validator against the exact development callback URL. Keep routing disabled even after validation succeeds until the call/SMS route contract is implemented.
